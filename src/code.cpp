...
Rcpp::List refit_LA_ADMM_export(const int& it_out, const int& it_in , const arma::mat& S, const arma::mat& A,
  ...
  // The remainder are the same inputs as the ones used in the refit_ADMM_block_new function
...
LA_ADMM_out refit_LA_ADMM_new(const int& it_out, const int& it_in , const arma::mat& S, const arma::mat& A,
  ...
  // it_out : scalar, T_stages of LA-ADMM algorithm
  // it_in : scalar, maximum number of iterations of ADMM algorithm
  // The remainder are the same inputs as the ones used in the refit_ADMM_block_new function. This function is for internal use
...
LA_ADMM_out LA_ADMM_taglasso(const int& it_out, const int& it_in , const arma::mat& S, const arma::mat& A,
  ...
  // it_out : scalar, T_stages of LA-ADMM algorithm
  // it_in : scalar, maximum number of iterations of ADMM algorithm
  // The remainder are the same inputs as the ones used in the ADMM_taglasso_block function. Function for internal use
...
Areduced_out determine_A_reduced(const arma::mat& Gamma, const arma::mat& A){
  ...
  int nnodes = A.n_cols;
  arma::mat AZ = A.col(nnodes-1);
  arma::rowvec newrow = Gamma.row(0);
  for(int it=(nnodes-2); it > -1 ; -- it){
...
int determine_dfs(arma::mat A, const double& tol = 1e-06){
  arma::mat Aabs = abs(A);
  A.elem( find(Aabs < tol) ).zeros();
  arma::mat A_unique = unique(A);
  arma::vec A_unique_nz = nonzeros(A_unique);
...
Rcpp::List ADMM_taglasso_cv_parallel(const arma::vec& fold_vector, const arma::vec& lambda1_vector, const arma::vec& lambda1_index_vector,
  ...
  // Input
  // fold_vector : vector of length Kfold*l1length*l2length indicating the folds
  // lambda1_vector : vector of length Kfold*l1length*l2length with values of aggregation tuning parameter
  // lambda1_index_vector : vector of length Kfold*l1length*l2length indexing the aggregation tuning parameter
  // lambda2_vector : vector of length Kfold*l1length*l2length with values of sparsity tuning parameter
  // lambda2_index_vector : vector of length Kfold*l1length*l2length indexing the sparsity tuning parameter
  // Sin : Sample covariance matrices on training data
  // Sout : Sample covariance matrices on test data
  ...
  // it_out : scalar, T_stages of LA-ADMM algorithm
  // it_in : scalar, maximum number of iterations of ADMM algorithm
  // it_out_refit : scalar, T_stages of LA-ADMM algorithm for refitting
  // it_in_refit : scalar, maximum number of iterations of ADMM algorithm for refitting
  // do_parallel: parallel computing with OpenMP or not
  // nc: number of cores to be used for parallel computing

  // Function : Cross-validation for selecting the regularization parameters lambda1 and lambda2
  ...
  prelim_A_out prelimoutA = preliminaries_for_taglasso_A(A);
  ...
  #pragma omp parallel for schedule(static) default(none) private(iall, LAADMMout_fit, omP, fitAZ, AZprelims, refit) shared(size_fold_lambdas, it_out, it_in, Sin, fold_vector, A, prelimoutA, lambda1_vector, lambda2_vector, rho, pendiag, p, nnodes, it_out_refit, it_in_refit, l1check, l2check, obj, dfs, sparsity, clusters, lambda1_index_vector, lambda2_index_vector, Sout)
    ...
    for(iall=0; iall < size_fold_lambdas; ++ iall){
      ...
      AZprelims = preliminaries_for_refit(fitAZ.AZ);
